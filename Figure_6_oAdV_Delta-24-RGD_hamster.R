...
p_early <- if (
  ...
  wilcox.test(
    early_struct$`In triad`,
    early_struct$`Outside triad`,
    paired = TRUE
  )$p.value
} else {
  NA_real_
...
terminal_struct <- stats_struct |>
  dplyr::filter(
    Timepoint == "Terminal"
  )

p_terminal <- if (
  nrow(terminal_struct) >= 3
) {
  wilcox.test(
    terminal_struct$`In triad`,
    terminal_struct$`Outside triad`,
    paired = TRUE
  )$p.value
} else {
  NA_real_
...
annot_df <- data.frame(
  ...
  label = c(
    ifelse(
      is.na(p_early),
      "NA",
      format_p_plotmath(p_early)
    ),
    ifelse(
      is.na(p_terminal),
      "NA",
      format_p_plotmath(p_terminal)
    )
  )
)
...
save_pdf(
  "../plots/Fig6_Myeloid_Distance_to_Vasculature.pdf",
  p_struct,
  P_Width,
  P_Height
)
